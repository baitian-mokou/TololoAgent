...
def blocked(reason: str, eval_report_path: Path, shadow_dir: Path) -> Dict[str, Any]:
    return {
        "phase": "Phase 79",
        "mode": "wikidata_shadow_review_only_cli",
...
def build_review_report(*, eval_report_path: Path, shadow_dir: Path, review_only: bool) -> Dict[str, Any]:
    ...
    if not isinstance(eval_report, dict) or eval_report.get("ready") is not True:
        return blocked("phase78_eval_not_ready", eval_report_path, shadow_dir)
    mappings = []
...
def render_md(report: Dict[str, Any]) -> str:
    return "\n".join(
        [
            "# Wikidata shadow review-only query preview CLI",
            "",
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Explicit review-only Wikidata shadow query preview.")
    parser.add_argument("--review-only", action="store_true")
    parser.add_argument("--shadow-dir", default=str(ROOT / "data" / "triples_shadow" / SOURCE_ID))
    parser.add_argument("--eval-report-json", default=str(ROOT / "evaluation" / "four_source_expansion" / "wikidata_shadow_readiness_eval_phase78.json"))
    parser.add_argument("--out-json", default=str(ROOT / "evaluation" / "four_source_expansion" / "wikidata_shadow_review_preview_cli_phase79.json"))
    parser.add_argument("--out-md", default=str(ROOT / "docs" / "wikidata_shadow_review_preview_cli_phase79.md"))
