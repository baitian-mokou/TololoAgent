...
- 通过：表示接受当前候选，让它可以进入下一步处理。通过时必须记录原因；GUI 会自动写入 `approved_in_gui` 这类说明。只有通过的项才会被 `apply_quality_patches.py` 读取。
- 暂缓：表示现在证据还不够，例如需要 NASA、ESA、论文或人工 ontology 设计。暂缓不会写正式 triples。
- 拒绝：表示不采用这条候选，例如只是来源精度差异，或者当前候选判断不可靠。拒绝也不会写正式 triples。

`pending` 是默认状态。队列刚生成时全部都是 pending，不会被合并。

## 修正建议

如果你认为系统候选值不对，但知道一个更合适的新值，可以在 GUI 的“我的修正建议”里填写：
...
保存修正建议只会写入 `quality_review_decisions.json`。它不会改正式 triples，不会 apply，不会写 Chroma，也不会写 Neo4j。

修正建议有几个状态：

- `none`：没有修正建议。
- `draft`：填了修正值，但还没有写原因。草稿只保存，不进入合并。
- `proposed`：修正值和原因都已填写，可以在 dry-run 报告里看到 diff，但默认仍不能合并。
- `validated`：已经额外确认，可以进入显式 value change 流程。
- `rejected`：这条修正建议不采用。

简单说：修正建议 = “我认为应该改成另一个值”，但它还没有写入正式库。
...
高风险 value change 即使已经通过，也只会生成 apply plan，不会直接改正式值。要允许改值必须同时满足：`human_decision=approved`、`revision_status=validated`、`safe_to_apply=true`，并且显式传入 `--allow-value-change` 和 `--apply-value-changes`。这一步应先由人确认来源证据、备份和回滚方案。
